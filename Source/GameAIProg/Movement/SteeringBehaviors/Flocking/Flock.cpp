...
Flock::Flock(UWorld *pWorld, TSubclassOf<ASteeringAgent> AgentClass,
             int FlockSize, float WorldSize,
             ASteeringAgent *const pAgentToEvade, bool bTrimWorld)
    : pWorld{pWorld}, FlockSize{FlockSize}, pAgentToEvade{pAgentToEvade},
      WorldSize{WorldSize}, AgentClass{AgentClass} {
  ...
  for (int i = 0; i < FlockSize; ++i) {
    FVector spawnPos{FMath::RandRange(-halfSize, halfSize),
                     FMath::RandRange(-halfSize, halfSize), 90.f};
    Agents[i] = pWorld->SpawnActor<ASteeringAgent>(AgentClass, spawnPos,
                                                   FRotator::ZeroRotator);
    if (IsValid(Agents[i]))
      Agents[i]->SetActorTickEnabled(false); // manually driven from Flock::Tick
  }

  // create spatial partitioning grid (12x12), matching world size so cells
  // align with visible play area
  int nrCells = 12;
  pCellSpace = std::make_unique<CellSpace>(pWorld, WorldSize, WorldSize,
                                           nrCells, nrCells, FlockSize);
  ...
  // wander has per-agent mutable state (angle + target), so one instance per
  // agent is required
  ...
  for (int i = 0; i < FlockSize; ++i) {
    ...
    pBlendedSteeringPerAgent[i] = std::make_unique<BlendedSteering>(
    ...
    pPrioritySteeringPerAgent[i] =
        std::make_unique<PrioritySteering>(std::vector<ISteeringBehavior *>{
            pEvadeBehavior.get(),
...
void Flock::Tick(float DeltaTime) {
  ...
  for (int i = 0; i < Agents.Num(); ++i) {
    ...
    // update flat grid cells when using cell space for neighbor queries
    if (pCellSpace && bUseSpacePartitioning && !bUseHISP) {
...
void Flock::RenderDebug() {
  if (DebugRenderSteering) {
    ...
    for (ASteeringAgent *agent : Agents) {
      ...
      DrawDebugLine(pWorld, agentPos, velEnd, FColor::Cyan, false, -1.f, 0,
                    3.f);
  ...
  if (DebugRenderPartitions) {
    if (bUseHISP && pQuadTree) {
      ...
      // rebuild for accurate display counts (same approach as
      // Level_SpacePartitioning)
...
#pragma region UI
  // UI
  {
    ...
    // use agent 0's weights for display; setters propagate the change to all
    // agents
    if (pBlendedSteeringPerAgent.Num() > 0 && pBlendedSteeringPerAgent[0]) {
      ...
      auto SetAllWeights = [&](int idx, float v) {
        for (auto &blend : pBlendedSteeringPerAgent)
          if (blend)
            blend->GetWeightedBehaviorsRef()[idx].Weight = v;
      };

      ImGuiHelpers::ImGuiSliderFloatWithSetter(
          "Cohesion", w0[0].Weight, 0.f, 1.f,
          [&](float v) { SetAllWeights(0, v); });
      ImGuiHelpers::ImGuiSliderFloatWithSetter(
          "Separation", w0[1].Weight, 0.f, 1.f,
          [&](float v) { SetAllWeights(1, v); });
      ImGuiHelpers::ImGuiSliderFloatWithSetter(
          "Alignment", w0[2].Weight, 0.f, 1.f,
          [&](float v) { SetAllWeights(2, v); });
      ImGuiHelpers::ImGuiSliderFloatWithSetter(
          "Seek", w0[3].Weight, 0.f, 1.f,
          [&](float v) { SetAllWeights(3, v); });
      ImGuiHelpers::ImGuiSliderFloatWithSetter(
          "Wander", w0[4].Weight, 0.f, 1.f,
          [&](float v) { SetAllWeights(4, v); });
    }
...
void Flock::RenderNeighborhood() {
  ...
  DrawDebugCircle(pWorld, firstPos, NeighborhoodRadius, 32, FColor::Yellow,
                  false, -1.f, 0, 3.f, FVector::YAxisVector,
                  FVector::XAxisVector);

  // query bounding box around the agent
  const float z = 91.f;
  FVector bl{agentPos2D.X - NeighborhoodRadius,
             agentPos2D.Y - NeighborhoodRadius, z};
  FVector tl{agentPos2D.X - NeighborhoodRadius,
             agentPos2D.Y + NeighborhoodRadius, z};
  FVector tr{agentPos2D.X + NeighborhoodRadius,
             agentPos2D.Y + NeighborhoodRadius, z};
  FVector br{agentPos2D.X + NeighborhoodRadius,
             agentPos2D.Y - NeighborhoodRadius, z};
  ...
  for (int i = 0; i < NrOfNeighbors; ++i) {
    if (!IsValid(Neighbors[i]))
      continue;
    DrawDebugLine(pWorld, firstPos, Neighbors[i]->GetActorLocation(),
